                  ...
                  <div className="glass p-5 rounded-2xl border border-pink-500/30 bg-zinc-900">
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        {selectedVenue.bookingEmail ? (
                          <a 
                            href={`mailto:${selectedVenue.bookingEmail}`} 
                            className="block p-3 bg-zinc-950 hover:bg-zinc-900 rounded-xl text-xs text-pink-400 hover:text-pink-300 font-bold border border-zinc-800 truncate transition-colors"
                          >
                            {selectedVenue.bookingEmail}
                          </a>
                        ) : (
                          <div className="p-3 bg-zinc-950 rounded-xl text-xs text-zinc-500 border border-zinc-800 truncate">
                            No email found
                          </div>
                        )}
                      </div>
                      <div>
                        ...
                        <textarea 
                          ...
                          defaultValue={`Hi ${selectedVenue.contactName || "Booking Manager"},\n\nI'm looking to book a show at ${selectedVenue.name} in late 2025. We noticed you have some availability.\n\nBest,\n[Your Name]`}
                        />
                      </div>
                      <button 
                      ...
                      >
                        {bookingSending ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : "Send Booking Request"}
                      </button>
                      ...
                      {selectedVenue.bookingEmail && (
                        <button
                          onClick={() => {
                            const messageVal = (document.getElementById("booking-message-textarea") as HTMLTextAreaElement)?.value || "";
                            window.location.href = `mailto:${selectedVenue.bookingEmail}?subject=Booking Inquiry: ${selectedVenue.name}&body=${encodeURIComponent(messageVal)}`;
                          }}
                          className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-black uppercase text-[10px] rounded-xl transition-all shadow-lg shadow-indigo-600/20 flex items-center justify-center gap-1.5"
                        >
                          <Mail className="w-3.5 h-3.5" /> Open in Mail App (mailto)
                        </button>
                      )}
                    </div>
                  </div>
              ...
              <div className="space-y-6">
                ...
                {showCounter && (
                  <div className="space-y-4">
                    {/* Global Reach Widget */}
                    <div className="glass bg-[#12121a]/90 backdrop-blur-3xl border border-pink-500/20 rounded-2xl p-4 shadow-[0_0_30px_rgba(236,72,153,0.05)] relative overflow-hidden">
                      <div className="absolute -top-10 -right-10 w-24 h-24 bg-cyan-500/20 rounded-full blur-3xl pointer-events-none"></div>
                      <h3 className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-3 flex justify-between items-center">
                        Global Reach <Globe className="w-3 h-3 text-cyan-400" />
                      </h3>
                      <div className="flex items-center gap-6 relative z-10">
                        <div className="flex flex-col">
                          <span className="text-[9px] text-zinc-500 font-bold mb-0.5">LIVE VENUES</span>
                          <span className="text-2xl font-black text-white tabular-nums tracking-tighter">{venues.length}</span>
                        </div>
                        <div className="flex flex-col">
                          <span className="text-[9px] text-zinc-500 font-bold mb-0.5">ARTISTS</span>
                          <span className="text-2xl font-black text-white tabular-nums tracking-tighter">{bands.length}</span>
                        </div>
                      </div>
                    </div>

                    {/* Creative Infrastructure Widget */}
                    <div className="glass bg-[#12121a]/90 backdrop-blur-3xl border border-orange-500/20 rounded-2xl p-4 shadow-[0_0_30px_rgba(249,115,22,0.05)] relative overflow-hidden">
                      <div className="absolute -bottom-10 -left-10 w-24 h-24 bg-orange-500/20 rounded-full blur-3xl pointer-events-none"></div>
                      <h3 className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-3 flex justify-between items-center">
                        Creative Infrastructure <Building2 className="w-3 h-3 text-orange-400" />
                      </h3>
                      <div className="flex items-center gap-5 relative z-10">
                        <div className="flex flex-col">
                          <span className="text-[9px] text-zinc-500 font-bold mb-0.5">STUDIOS</span>
                          <span className="text-2xl font-black text-white tabular-nums tracking-tighter">{studios}</span>
                        </div>
                        <div className="flex flex-col">
                          <span className="text-[9px] text-zinc-500 font-bold mb-0.5">REHEARSAL</span>
                          <span className="text-2xl font-black text-white tabular-nums tracking-tighter">{rehearsals}</span>
                        </div>
                        <div className="flex flex-col">
                          <span className="text-[9px] text-zinc-500 font-bold mb-0.5">STORES</span>
                          <span className="text-2xl font-black text-white tabular-nums tracking-tighter">{shops}</span>
                        </div>
                      </div>
                    </div>

                    {/* Circuit Pulse Widget */}
                    <div className="glass bg-[#12121a]/90 backdrop-blur-3xl border border-indigo-500/20 rounded-2xl p-4 shadow-[0_0_30px_rgba(99,102,241,0.05)] relative overflow-hidden flex items-center justify-between">
                      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full bg-indigo-500/10 blur-3xl pointer-events-none"></div>
                      <div className="relative z-10">
                        <h3 className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1 flex items-center gap-1.5">
                          Total Pulse <Activity className="w-3 h-3 text-indigo-400" />
                        </h3>
                        <span className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-indigo-400 to-pink-400 tabular-nums tracking-tighter">
                          {totalIntel.toLocaleString()} Nodes
                        </span>
                      </div>
                      <div className="relative z-10 p-2 bg-indigo-500/10 rounded-xl">
